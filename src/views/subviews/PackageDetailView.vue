            ...
            <v-simple-table>
                <template v-slot:default>
                    ...
                    <tr v-for="item in flights" :key="item.name">
                        ...
                        <td>{{ lowestPrice(item) }}<br>{{ lowestPriceAllIn(item) }}</td>
...
export default {
   ...
   mounted() {
    this.$store.dispatch('fetchPackageByID',this.$router.currentRoute.params.packageID)
    .then(result => {
        this.package = result.data();

        this.$store.dispatch('fetchAttractionByDestinationID',this.package.destination)
        .then(result => {
    ...
    this.$store.dispatch('fetchFlightByPackageID',this.$router.currentRoute.params.packageID)
    .then(result => {
    ...
    this.$store.dispatch('fetchMealByPackageID',this.$router.currentRoute.params.packageID)
    .then(result => {
        this.meal = result[0];
    ...
    this.$store.dispatch('fetchGroundByPackageID',this.$router.currentRoute.params.packageID)
    .then(result => {
    ...
    this.$store.dispatch('fetchBagAndInsuranceByPackageID',this.$router.currentRoute.params.packageID)
    .then(result => {
    ...
    this.$store.dispatch('fetchTourLeaderByPackageID',this.$router.currentRoute.params.packageID)
    .then(result => {
  ...
  methods: {
    ...
    calculateFare(item){
        let groundPrice = (Number(item) + Number(this.package.tipping)) * Number(this.package.days);

        let netPrice = Number(this.selectedTour.allInTicketFare) + groundPrice + Number(this.tourLeader.price) + Number(this.bagAndInsurance.price) 
    ...
    calculateAllIn(item){
        ...
        let netPrice = Number(this.selectedTour.allInTicketFare) + groundPrice + Number(this.selectedTour.allInFlightTax) + Number(this.tourLeader.price) + Number(this.bagAndInsurance.price) 
    ...
    lowestPrice(item){
        let groundPrice = (Math.min.apply(Math, this.ground.child.map(a => a.price)) + Number(this.package.tipping)) * Number(this.package.days);

        let netPrice = Number(item.allInTicketFare) + groundPrice + Number(this.tourLeader.price) + Number(this.bagAndInsurance.price) 
    ...
    lowestPriceAllIn(item){
        ...
        let netPrice = Number(item.allInTicketFare) + Number(item.allInFlightTax) + groundPrice + Number(this.tourLeader.price) + Number(this.bagAndInsurance.price) 
